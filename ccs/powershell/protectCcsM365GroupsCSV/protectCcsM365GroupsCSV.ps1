...
if(!$csvFile -and $autoprotectCount -gt 0){
    Write-Host "Finding groups to autoprotect"
    $foundObjects = 0
    ...
    while($foundObjects -lt $autoprotectCount){
        $search = api get -v2 "data-protect/search/objects?environments=kO365&o365ObjectTypes=kGroup&regionIds=$region&sourceIds=$rootSourceId&count=$searchCount&isProtected=false&searchString=*"
        foreach($obj in $search.objects | Where-Object {! $_.groupParams.PSObject.Properties['isTeamsGroup'] -or $_.groupParams.isTeamsGroup -ne $True}){
            $foundObjects += 1
    ...
    if($foundObjects -lt $autoprotectCount){
        Write-Host "*** $foundObjects groups to autoprotect"
    }
}
...
foreach($obj in $objectsToAdd){
    ...
    if($script:smtpIndex.ContainsKey($objSMTP)){
        ...
        $search = api get -v2 "data-protect/search/objects?regionIds=$region&sourceIds=$rootSourceId&environments=kO365&o365ObjectTypes=kGroup&includeTenants=true&count=999&searchString=$objName"
    ...
    if($objId -and $objId -in $script:unprotectedIndex){
        ...
        if($useMBS){
            $protectionParams.objects[0].environment = "kO365GroupssCSM"
        }else{
        ...
        Write-Host "Group $objSMTP not found" -ForegroundColor Yellow
    }
}
