...
if(!$csvFile -and $autoprotectCount -gt 0){
    Write-Host "Finding teams to autoprotect"
    $foundObjects = 0
    ...
    while($foundObjects -lt $autoprotectCount){
        $search = api get -v2 "data-protect/search/objects?environments=kO365&o365ObjectTypes=kTeam&regionIds=$region&sourceIds=$rootSourceId&count=$searchCount&isProtected=false&searchString=*"
    ...
    if($foundObjects -lt $autoprotectCount){
        Write-Host "*** $foundObjects teams to autoprotect"
    }
}
