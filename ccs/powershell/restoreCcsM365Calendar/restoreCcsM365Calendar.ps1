        ...
        foreach($result in $exactMatch | Where-Object {$_.name -eq $objName}){
            ...
            foreach($objectProtectionInfo in $result.objectProtectionInfos){
                ...
                if($snapshots -and $snapshots.Count -gt 0){
                    if($listDates){
                        Write-Host "RecoverDates Available:"
                        $snapshots | Format-Table -Property @{label="Recovery Date"; expression={usecsToDate $_.runStartTimeUsecs}}
                        exit
                    }
                    if($recoverDate){
                    ...
                    $calendarSearch = api post -v2 "data-protect/search/indexed-objects?regionId=$objectRegionId" $calendarQuery
                    $calendar = $calendarSearch.emails | Where-Object {$_.folderName -eq 'Calendar'}
                    if($calendar){
                        $calendarId = $calendar[0].id
                    }else{
                        Write-Host "No calendar for $objName" -ForegroundColor Yellow
                        continue
                    }
