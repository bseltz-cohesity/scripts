...
foreach($obj in $objectsToAdd){
    ...
    if($script:smtpIndex.ContainsKey($objSMTP)){
        ...
        $search = api get -v2 "data-protect/search/objects?environments=kO365&o365ObjectTypes=kGroup&regionIds=$region&sourceIds=$rootSourceId&count=999&searchString=$objName"
    ...
    if($objId -and $objId -in $script:protectedIndex){
        ...
        Write-Host "Group $objSMTP not protected" -ForegroundColor Magenta
    }else{
