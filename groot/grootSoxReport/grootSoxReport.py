...
title = 'Sox Report (%s)' % cluster['name']
now = datetime.now()
...
rows = cur.fetchall()
for row in rows:
    (jobName, objectName, sourceType, sourceName, jobStatus, taskType, policyName, fullincr, duration, startTime, endTime, expiryDate, slaviolated) = row
    if namefilter is None or fnmatch(objectName.lower(), namefilter.lower()) or fnmatch(sourceName.lower(), namefilter.lower()) or fnmatch(jobName.lower(), namefilter.lower()):
        html += '''<tr class="%s">
                ...
                <td>%s</td>
                <td>%s</td>
            </tr>''' % (jobStatus, jobName, objectName, sourceType, sourceName, jobStatus, policyName, fullincr, duration, startTime, endTime, expiryDate, slaviolated)
...
if namefilter is not None:
    ...
    outfileName = 'soxReport-%s-%s.html' % (cluster['name'], namefilterencoded)
else:
    outfileName = 'soxReport-%s.html' % cluster['name']
