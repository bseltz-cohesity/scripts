#!/usr/bin/env python3
"""
Gflag Manager local proxy
============================
A tiny local helper that lets the companion gflag-manager.html page talk to
a Cohesity cluster (or Helios/MCM) from a browser.
...
Usage:
    python3 gflag-manager-proxy.py
    (then open gflag-manager.html in your browser)

The proxy listens on 127.0.0.1 only - it is not reachable from the network.
It holds one login session at a time, mirroring how the gflags.py / pyhesity
...
# ---------------------------------------------------------------------------
# session state - one global session, mirrors pyhesity's global auth state
# ---------------------------------------------------------------------------
STATE = {
    'connected': False,
    'vip': None,
...
# ---------------------------------------------------------------------------
# business logic - mirrors the relevant parts of pyhesity.py / gflags.py
# ---------------------------------------------------------------------------
def do_login(body):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, status, payload):
        ...
        # cache a response, or a repeated GET (e.g. /api/flaglist?service=...) can come
        # back from cache instead of hitting the proxy/cluster again.
        self.send_header('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0')
        self.send_header('Pragma', 'no-cache')
        self.end_headers()
        self.wfile.write(body)
        # log_message() is silenced below for normal traffic, but errors are always
        # printed to the console this script is running in - useful for troubleshooting
        # even without editing the code.
        if status >= 400:
            print('[%s] %s %s -> %s : %s' % (
    ...
    def do_GET(self):
        ...
        elif path == '/api/gflags':
            status, payload = do_get_gflags()
            self._send(status, payload)
        elif path == '/api/services':
            status, payload = do_list_services()
            self._send(status, payload)
        elif path == '/api/flaglist':
    ...
    def do_POST(self):
        ...
        if self.path == '/api/login':
            ...
            status, payload = do_restart(body)
            self._send(status, payload)
...
if __name__ == '__main__':
    ...
    print('Gflag Manager local proxy running at http://127.0.0.1:%d' % PORT)
    print('Open gflag-manager.html in your browser - it talks only to this proxy.')
