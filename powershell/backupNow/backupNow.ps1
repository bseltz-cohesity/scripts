...
if($job){
    ...
    if($objects){
        ...
        if($environment -in @('kOracle', 'kSQL')){
            ...
            $sources = api get "protectionSources/virtualMachines?protected=true&pruneNonCriticalInfo=true&useCachedData=$cacheSetting&vCenterId=$($v1Job.parentSourceId)" -timeout $timeoutSec
