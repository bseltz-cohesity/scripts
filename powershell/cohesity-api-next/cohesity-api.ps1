...
[CmdletBinding()]
param (
    [Parameter()][switch]$updatePwd = $false,
    [Parameter()][switch]$updateApiKey = $false
)

# . . . . . . . . . . . . . . . . . . .
#  PowerShell Module for Cohesity API
#  Version 2021.02.10 - Brian Seltzer
# . . . . . . . . . . . . . . . . . . .
#
...
# 2021.03.26 - added direct updatePassword function and apiKey unique password storage
# 
# . . . . . . . . . . . . . . . . . . .
$versionCohesityAPI = '2021.03.26'
...
function storePasswordInFile($vip='helios.cohesity.com', $username='helios', $domain='local', $passwd=$null, [switch]$useApiKey){
    Set-CohesityAPIPassword -vip $vip -username $username -domain $domain -passwd $passwd -useApiKet $useApiKey
}
...
if($updatePwd -or $updateApiKey){
    if(!$vip){
        $default = 'helios.cohesity.com'
        if (!($vip = Read-Host "vip ($default)")) { $vip = $default }
    }
    if(!$username){
        $default = 'helios'
        if (!($username = Read-Host "username ($default)")) { $username = $default }
    }
    if(!$domain){
        $default = 'local'
        if (!($domain = Read-Host "domain ($default)")) { $domain = $default }
    }
    $null = Set-CohesityAPIPassword -vip $vip -username $username -domain $domain -passwd $passwd -quiet -useApiKey $useApiKey
    if($updateApiKey){
