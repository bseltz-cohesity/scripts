# . . . . . . . . . . . . . . . . . . .
#  PowerShell Module for Cohesity API - Tls13 version
#  Version 2026.08.27 - Brian Seltzer
# . . . . . . . . . . . . . . . . . . .
#
...
function apiauth([string] $vip='helios.cohesity.com',
                 [string] $username = 'helios',
                 [string] $passwd = $null,
        ...
        if($clientId -and $directoryId -and $scope){
            ...
            if($token){
                ...
                try{
                    ...
                    $heliosAllClusters = __auth -method Get -url $URL -header $header -timeout $timeout
            ...
            if(! $noDomain){
                try{
                    ...
                    if($changePassword -eq $True){
                        ...
                        $null = __apicall -method Put -url $URL -body ($user.user | ConvertTo-Json) -timeout $timeout    
                        $body = ConvertTo-Json @{
                            'domain' = $domain;
                            'username' = $username;
                ...
                if($mfaCode -or $emailMfaCode){
                    ...
                    if($emailMfaCode){
                        ...
                        $null = __apicall -method Post -url $url -timeout $timeout
...
function __connected($quiet=$false){
    $cohesity_api.authorized = $true
...
function __auth($method, $url, $body=$null, $header=$cohesity_api.header, $timeout=300){
    ...
    if($null -ne $body){
        if($PSVersionTable.PSEdition -eq 'Core'){
            $auth = Invoke-RestMethod -Method $method -Uri $url -header $header -Body $body -SkipCertificateCheck -UserAgent $cohesity_api.userAgent -TimeoutSec $timeout -SessionVariable session -ContentType "application/json; charset=utf-8" -SslProtocol Tls13 -UseBasicParsing
        }else{
        ...
        if($PSVersionTable.PSEdition -eq 'Core'){
            $auth = Invoke-RestMethod -Method $method -Uri $URL -Header $header -TimeoutSec $timeout -UserAgent $cohesity_api.userAgent -SslProtocol Tls13 -SkipCertificateCheck -SessionVariable session -ContentType "application/json; charset=utf-8" -UseBasicParsing
        }else{
...
function __apicall($method, $url, $body=$null, $header=$cohesity_api.header, $timeout=300){
    if($null -ne $body){
        if($PSVersionTable.PSEdition -eq 'Core'){
            $response = Invoke-RestMethod -Method $method -Uri $url -header $header -Body $body -SkipCertificateCheck -UserAgent $cohesity_api.userAgent -TimeoutSec $timeout  -WebSession $cohesity_api.session -ContentType "application/json; charset=utf-8" -SslProtocol Tls13  -UseBasicParsing
        }else{
        ...
        if($PSVersionTable.PSEdition -eq 'Core'){
            $response = Invoke-RestMethod -Method $method -Uri $URL -Header $header -TimeoutSec $timeout -UserAgent $cohesity_api.userAgent -SslProtocol Tls13 -SkipCertificateCheck  -WebSession $cohesity_api.session -ContentType "application/json; charset=utf-8" -UseBasicParsing
        }else{
...
function accessCluster($remoteClusterName=$null){
    if($null -eq $cohesity_api.heliosConnectedClusters){
        if($null -eq $remoteClusterName -or $remoteClusterName -eq '-'){
            $cohesity_api.header.Remove('clusterId')
            $cohesity_api.session.Headers.remove('clusterId')       
        }else{
...
function fileDownload($uri, $fileName, [switch]$v2, [switch]$quiet){
    ...
    try {
        ...
        if($PSVersionTable.PSEdition -eq 'Core'){
            Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile $fileName -Header $cohesity_api.header -WebSession $cohesity_api.session -UserAgent $cohesity_api.userAgent -SslProtocol Tls13 -SkipCertificateCheck
        }else{
...
function fileUpload($uri, $fileName, [switch]$v2){
    ...
    try {
        ...
        if($PSVersionTable.PSEdition -eq 'Core'){
            $result = Invoke-WebRequest -UseBasicParsing -Method Post -Uri $url -InFile $fileName -Header $cohesity_api.header -WebSession $cohesity_api.session -UserAgent $cohesity_api.userAgent -SslProtocol Tls13 -SkipCertificateCheck
        }else{
...
function readCache{
    ...
    param(
        ...
        [Parameter()][int]$maxAgeMinutes = 60
    )
    $nowUsecs = dateToUsecs
    if(Test-Path -Path $cacheFile -PathType Leaf){
        $response = Get-Content $cacheFile | ConvertFrom-Json
        if($response.PSObject.Properties['timestamp']){
            $cacheAge = $nowUsecs - $response.timestamp
            if($cacheAge -le ($maxAgeMinutes *  60000000)){
...
function writeCache{
    ...
    $cache = @{'content' = $content;
               'timestamp' = $nowUsecs}
    $cache | ConvertTo-Json -Depth 99 | Out-File -FilePath $cacheFile
    return $null
}
