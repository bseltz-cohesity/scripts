...
param (
    ...
    [Parameter(Mandatory=$True)][string]$adDomain,
...
foreach($thisPrincipal in $principalNames){
    # find principal
    $existingUser = $users | Where-Object {$_.username -eq $thisPrincipal -and $_.domain -eq $adDomain}
    $existingGroup = $groups | Where-Object {$_.name -eq $thisPrincipal -and $_.domain -eq $adDomain}
    if(!$remove){
