# usage: ./protectAix.ps1 -vip mycluster -username myusername -jobName 'My Job' -serverList ./servers.txt -exclusionList ./exclusions.txt
...
foreach($server in $serversToAdd){
    ...
    if($node){
        if($node.protectionSource.physicalProtectionSource.hostType -eq 'kAix'){
            $sourceId = $node.protectionSource.id
            $sourceIds += $sourceId
        }else{
            Write-Warning "$server is not a AIX host"
        }
    }else{
