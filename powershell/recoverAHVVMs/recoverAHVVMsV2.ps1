...
if($ahvSourceName){
    ...
    if(!$detachNetwork){
        ...
        if($preserveMacAddress){
            $restoreParams.acropolisParams.recoverVmParams.acropolisTargetParams.recoveryTargetConfig.newSourceConfig.networkConfig.newNetworkConfig.preserveMacAddress = $True
        }
    }
}else{
