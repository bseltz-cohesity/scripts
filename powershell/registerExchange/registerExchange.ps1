...
foreach($server in $servers){
    ...
    if($registeredSource){
        ...
        if($registeredSource.applications | Where-Object environment -eq kExchange){
    ...
    # register Exchange
    if(!$alreadyExchange){
        Write-Host ": registering as Exchange"
        $regExchange = @{"ownerEntity" = @{"id" = $entityId}; "appEnvVec" = @(17)}
        $result = api post /applicationSourceRegistration $regExchange
    }else{
