...
.\viewDRclone.ps1 -vip $drCluster -username $userName -domain $domain -viewList ./myviews.txt -inPath "$metaDataPath\$primaryCluster" -policyName $drPolicyName
# toggle dfs folder targets
.\dfsFailover.ps1 -nameSpace $dfsNameSpace -shareList .\migratedShares.txt -disableTarget $primaryDFSPath -enableTarget $drDFSPath
# toggle cnames and SPNs
.\cnameFailover.ps1 -cname $cname -oldHost $primaryArecord -newHost $drArecord -domain $cnameDomain
