...
# using helios
# .\viewDR.ps1 -helios -targetCluster $targetCluster -username $userName -domain $userDomain -viewList $viewList -unplannedFailover
# using cluster direct
.\viewDR.ps1 -targetCluster $targetCluster -username $userName -domain $userDomain -viewList $viewList -unplannedFailover
# using mcm
# .\viewDR.ps1 -mcm $mcm -targetCluster $targetCluster -username $userName -domain $userDomain -viewList $viewList -unplannedFailover
