...
def apiauth(vip='helios.cohesity.com', username='helios', domain='local', password=None, newPassword=None,
    ...
    if entraId is True and vip.lower() in HELIOSENDPOINTS:
        COHESITY_API['USING_HELIOS'] = True
        ...
        while retrycounter <= 10:
            retrycounter += 1
            response = COHESITY_API['SESSION'].get(URL, headers=COHESITY_API['HEADER'], verify=False, timeout=timeout)
        ...
        while retrycounter <= 10:
            retrycounter += 1
            try:
                # print('*** starting authentication attempt')
                                ...
                                if response != '':
                                    if response.status_code == 201:
                                        ...
                                        if response.status_code == 400:
                                            if response.json()['message'].lower() == 'please specify the mandatory parameters.':
                                                print(response.json()['message'])
                                                COHESITY_API['AUTHENTICATED'] = False
                                                COHESITY_API['LAST_ERROR'] = response.json()['message']
                                                __writelog('Error %s' % response.json()['message'])
                                                return None
                                        # try session auth
                                            ...
                                            try:
                                                ...
                                                if response != '':
                                                    if response.status_code == 201:
                                                        ...
                                                        COHESITY_API['AUTHENTICATED'] = False
                                                        if response.status_code == 400 and 'mandatory parameters' in response.json()['message'].lower():
                                                            print(response.json()['message'])
                                                            return None
...
def impersonate(tenantId):
    if COHESITY_API['AUTHENTICATED'] is True:
        if COHESITY_API['USING_HELIOS'] is True:
            tenants = api('get', 'users/tenant-access', mcmv2=True)
            if tenants is not None and 'tenantAccesses' in tenants:
                thistenant = [t for t in tenants['tenantAccesses'] if t['tenantName'].lower() == tenantId.lower()]
        else:
    ...
    if method in APIMETHODS:
        ...
        while True:
            try:
                ...
                if quiet is None:
                    print(e)
                # if 'magneto' in str(e).lower() or 'timeout' in str(e).lower():
                #     retrycounter += 1
                #     if retrycounter >= 10:
                #         pass
                #     else:
                ...
                if responsejson is not None:
                    ...
                    try:
                        if quiet is None:
                            ...
                            # print('*** response text %s' % responsetext)
                            return responsetext
                        else:
                            return None
                    except Exception:
