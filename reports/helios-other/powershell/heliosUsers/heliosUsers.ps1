...
"""Email Address"",""Username"",""Firstname"",""Lastname"",""Roles"",""Clusters"",""Last Updated""" | Out-File -FilePath $outfile
...
foreach($user in $users){
    ...
    """$($user.emailAddress)"",""$($user.username)"",""$($user.firstName)"",""$($user.lastName)"",""$($userRoles -join '; ')"",""$($userClusters -join '; ')"",""$(usecsToDate ($user.lastUpdatedTimeMsecs * 1000))""" | Out-File -FilePath $outfile -Append
}
