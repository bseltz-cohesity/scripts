...
# CSV output
$csvFileName = "heliosStoragePerObjectReport_$start_$end.csv"
"Cluster Name,Source Name,Object Name,Environment,Snapshots,Logical $unit,$unit Read,$unit Written,$unit Daily Change" | Out-File -FilePath $csvFileName

# HTML output
$htmlFileName = "heliosStoragePerObjectReport_$start_$end.html"

$title = "Helios Storage Per Object"
    ...
    while($moreRecords){
        ...
        foreach($object in $preview.component.data | Sort-Object -Property {$_.system}, {$_.objectName}){
            ...
            if($objectName){
                ...
                $dataRead = $object.sumSourceDeltaSizeBytes
...
foreach($uniqueKey in $stats.Keys | Sort-Object){
    ...
    $objectType = $stats[$uniqueKey].objectType
    $snapshots = $stats[$uniqueKey].snapshots
    $logicalSize = $stats[$uniqueKey].logicalSize
    $dataRead = toUnits $stats[$uniqueKey].dataRead
    $dataWritten = toUnits $stats[$uniqueKey].dataWritten
    $changeRate = toUnits $stats[$uniqueKey].changeRate
    ...
    $Global:html += '<tr style="border: 1px solid {9} background-color: {9}">
        ...
        <td>{7}</td>
        <td>{8}</td>
        </tr>' -f $clusterName, $sourceName, $objectName, $objectType, $snapshots, $logicalSize, $dataRead, $dataWritten, $changeRate, $Global:trColor
}
