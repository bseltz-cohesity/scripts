...
foreach($i in $preview.component.data | Sort-Object -Property systemName){
    ...
    $logicalData = toUnits $i.logicalUsageBytes
    $reduction = [math]::round($i.storageReduction, 1)
    $growth = toUnits $i.totalGrowthBytes
    $dailyGrowth = toUnits $i.avgDailyGrowthBytes
    $dailyPctGrowth = [math]::round($i.avgDailyGrowthRatePercent, 1)
    ...
    $Global:html += '<tr>
        ...
        <td>{7}</td>
        <td>{8}</td>
        </tr>' -f $clusterName, $capacity, $used, $pctUsed, $logicalData, $reduction, $growth, $dailyGrowth, $dailyPctGrowth            
}
