#!/usr/bin/env python
"""Helios v2 Protected Objects Report"""
...
filePrefix = "heliosProtectedObjectReport"
title = "Helios Protected Object Report"
reportNumber = '000'
...
for range in ranges:
    ...
    for cluster in clusters:
        ...
        for i in data:
            ...
            lastRunDate = ''
            if i['lastRunTime']:
                lastRunDate = usecsToDate(i['lastRunTime'], '%Y-%m-%d %H:%M')
            lastSuccess = ''
            if i['lastSuccessfulBackup']:
                lastSuccess = usecsToDate(i['lastSuccessfulBackup'], '%Y-%m-%d %H:%M')
            protected = i['protectionStatus']
...
for uniqueKey in sorted(stats.keys()):
    ...
    lastSuccess = stats[uniqueKey]['lastSuccess']
    ...
    html += '''<tr>
        ...
        <td>%s</td>
        <td>%s</td>
        </tr>''' % (protected, clusterName, sourceName, objectName, environment, jobName, policy, lastResult, lastRunDate, lastSuccess, numSnapshots, successful, unsuccessful)
