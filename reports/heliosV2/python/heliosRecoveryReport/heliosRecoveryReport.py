...
for range in ranges:
    ...
    for i in sorted(preview['component']['data'], key=lambda d: d['startTimeUsecs'], reverse=True):
        clusterName = i['system'].upper()
        status = i['status'][1:]
        startTime = usecsToDate(i['startTimeUsecs'], '%Y-%m-%d %H:%M')
        objectName = i['objectName']
        environment = i['environment']
        if(len(environment) > 0):
            environment = environment[1:]
        sourceName = i['sourceName']
        recoveryPoint = usecsToDate(i['recoveryPointUsecs'], '%Y-%m-%d %H:%M')
        ...
        taskName = i['taskName']
        taskUser = i['username']
        csv.write('"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' % (clusterName, status, startTime, objectName, environment, sourceName, recoveryPoint, duration, durationSecs, taskName, taskUser))
        html += '''<tr>
            ...
            <td>%s</td>
            <td>%s</td>
            </tr>''' % (clusterName, status, startTime, objectName, environment, sourceName, recoveryPoint, duration, durationSecs, taskName, taskUser)
