#!/usr/bin/env python
"""Helios v2 Storage Per Group Report"""
...
filePrefix = "heliosStoragePerGroupReport"
title = "Helios Storage Per Group Report"
reportNumber = 400
...
Storage Consumed (%s)
Growth (%s)
Daily Growth (%s)''' % (units, units, units, units, units)).split('\n')
...
for i in sorted(preview['component']['data'], key=lambda d: (d['system'].lower(), d['groupName'].lower())):

    clusterName = i['system'].upper()
    jobName = i['groupName']
    environment = i['environment'][1:]
    sourceName = ', '.join(i['sourceNames'])
    policy = i['policyName']
    successes = i['totalSuccessfulRuns']
    ...
    consumed = round(float(i['sumDataWrittenSizeBytes']) / multiplier, 1)
    growth = round(float(i['totalGrowthBytes']) / multiplier, 1)
    dailyGrowth = round(float(i['avgDailyGrowthRateBytes']) / multiplier, 1)
    csv.write('"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' % (clusterName, jobName, environment, sourceName, policy, successes, logicalSize, dataRead, consumed, growth, dailyGrowth))
    html += '''<tr>
        ...
        <td>%s</td>
        <td>%s</td>
        </tr>''' % (clusterName, jobName, environment, sourceName, policy, successes, logicalSize, dataRead, consumed, growth, dailyGrowth)
