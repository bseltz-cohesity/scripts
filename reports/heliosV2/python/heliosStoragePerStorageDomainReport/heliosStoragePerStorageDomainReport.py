#!/usr/bin/env python
"""Helios v2 Storage Per Storage Domain Report"""
...
for i in sorted(preview['component']['data'], key=lambda d: (d['systemName'].lower(), d['viewBoxName'].lower())):
    ...
    if i['lastStorageConsumedBytes'] > i['firstStorageConsumedBytes']:
        ...
        dailyPctGrowth = 0
    csv.write('"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' % (clusterName, sdName, logicalSize, dataIn, dataWritten, consumed, reduction, growth, dailyGrowth, dailyPctGrowth))
    html += '''<tr>
        ...
        <td>%s</td>
        <td>%s</td>
        </tr>''' % (clusterName, sdName, logicalSize, dataIn, dataWritten, consumed, reduction, growth, dailyGrowth, dailyPctGrowth)
