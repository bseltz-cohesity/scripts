#!/usr/bin/env python
"""Helios v2 Storage Per System Report"""
...
filePrefix = "heliosStoragePerSystemReport"
title = "Helios Storage Per System Report"
reportNumber = 800
...
for i in sorted(preview['component']['data'], key=lambda d: d['systemName'].lower()):
    ...
    capacity = round(float(i['totalCapacityBytes']) / multiplier, 1)
    used = round(float(i['physicalUsageBytes']) / multiplier, 1)
    pctUsed = round(i['usagePercent'], 1)
    logicalData = round(float(i['logicalUsageBytes']) / multiplier, 1)
    reduction = round(i['storageReduction'], 1)
    growth = round(float(i['totalGrowthBytes']) / multiplier, 1)
    dailyGrowth = round(float(i['avgDailyGrowthBytes']) / multiplier, 1)
    ...
    html += '''<tr>
        ...
        <td>%s</td>
        <td>%s</td>
        </tr>''' % (clusterName, capacity, used, pctUsed, logicalData, reduction, growth, dailyGrowth, dailyPctGrowth)
