...
# headings
# $headings = "Activity Type,Target,Start Time,End Time,Duration,status,slaStatus,snapshotStatus,objectName,sourceName,groupName,policyName,Object Type,backupType,System Name,Logical Size $unit,Data Read $unit,Data Written $unit,Organization Name,DataLock Expiry,Legal Hold,Snapshot Expiry"
                            ...
                            foreach($object in $run.objects){
                                ...
                                if($environment -notin @('kOracle', 'kSQL') -or ($environment -in @('kOracle', 'kSQL') -and $object.object.objectType -notin @('kDatabase'))){
                                    ...
                                    if(!$onHoldOnly -or $onLegalHold -eq $True){
                                        ...
                                        # get replication and archival activities
                                        if($object.PSObject.Properties['replicationInfo'] -and $object.replicationInfo.PSObject.Properties['replicationTargetResults']){
                                            ...
                                            foreach($result in $object.replicationInfo.replicationTargetResults){
                                                ...
                                                # 'Replication', $result.clusterName, $(dateToString $objectStartTime), $(dateToString $replicationEndTime), $replicationDurationSeconds, $result.status, $slaStatus, 'Active', $objectName, $registeredSourceName, $job.name, $policyName, $environment, $runType, $cluster.name, $objectLogicalSizeBytes, '-', '-', $tenant, $lockUntil, $onLegalHold, '-' -join "," | Out-File -FilePath $outfileName -Append
