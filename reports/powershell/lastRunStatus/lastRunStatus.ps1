...
$csvFile = $(Join-Path -Path $outputPath -ChildPath "lastRunStatus_$fileDate.csv")
$htmlFile = $(Join-Path -Path $outputPath -ChildPath "lastRunStatus_$fileDate.html")
...
<tr>
    ...
    <th>Last Status</th>
    <th>Paused</th>
    <th>Backed Up Last 24 Hours</th>
</tr>'

"Cluster Name,Job Name,Tenant,Policy,Last Run,Last Status,Paused,Backed Up Last 24 Hours" | Out-File -FilePath $csvFile -Encoding utf8
...
function reportJobs(){
    ...
    foreach($job in $jobs.protectionGroups | Sort-Object -Property name){
        ...
        if($lastStatus -eq 'Failed' -or $backedUpLast24 -eq $False){
        ...
        $Script:html += "<td>{0}</td>
              ...
              <td>{5}</td>
              <td>{6}</td>
              <td>{7}</td></tr>" -f $cluster.name, $job.name, $tenant, $policy.name, $(usecsToDate $lastRunUsecs), $lastStatus, $($job.isPaused), $backedUpLast24
        "$($cluster.name),$($job.name),$tenant,$($policy.name),$(usecsToDate $lastRunUsecs),$lastStatus,$($job.isPaused),$backedUpLast24" | Out-File -FilePath $csvFile -Append
    }    
}
