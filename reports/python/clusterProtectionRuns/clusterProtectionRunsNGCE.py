                                ...
                                for object in run['objects']:
                                    ...
                                    if len(objectnames) == 0 or objectName.lower() in [o.lower() for o in objectnames]:
                                        ...
                                        if environment not in ['kOracle', 'kSQL'] or object['object']['objectType'] != 'kHost':
                                            ...
                                            objectStartTime = usecsToDate(object['archivalInfo']['archivalTargetResults'][0]['startTimeUsecs'])
                                            objectEndTime = None
                                            objectDurationSeconds = int((nowUsecs - object['archivalInfo']['archivalTargetResults'][0]['startTimeUsecs']) / 1000000)
                                            if 'endTimeUsecs' in object['archivalInfo']['archivalTargetResults'][0]:
                                                objectEndTime = usecsToDate(object['archivalInfo']['archivalTargetResults'][0]['endTimeUsecs'])
                                                objectDurationSeconds = int((object['archivalInfo']['archivalTargetResults'][0]['endTimeUsecs'] - object['archivalInfo']['archivalTargetResults'][0]['startTimeUsecs']) / 1000000)
                                            objectLogicalSizeBytes = round(object['archivalInfo']['archivalTargetResults'][0]['stats'].get('logicalSizeBytes', 0) / multiplier, 1)
                                            objectBytesWritten = round(object['archivalInfo']['archivalTargetResults'][0]['stats'].get('physicalBytesTransferred', 0) / multiplier, 1)
                                            objectBytesRead = round(object['archivalInfo']['archivalTargetResults'][0]['stats'].get('bytesRead', 0) / multiplier, 1)
