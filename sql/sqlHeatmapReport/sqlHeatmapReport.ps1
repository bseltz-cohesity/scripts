...
$title = "SQL Heatmap Report for $($cluster.name)"
...
foreach($item in $report | Sort-Object -Property parentSourceName, name){
    $parentName = $item.parentSourceName
    $objectName = $item.name
    $objectType = $item.environment.subString(1)
    if($objectType -eq 'SQL'){
        "    $objectName"
        $trends = $item.trends
        $trendCells = @()
...
if($smtpServer -and $sendTo -and $sendFrom){
    ...
    foreach($toaddr in $sendTo){
        Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject $title -Attachments $fileName -WarningAction SilentlyContinue
    }
}